...
def clean_data(data):
    print(data.head())
    data = data.dropna(subset=["Timestamp", "Nitrogen01"])
    data = data[~data["Timestamp"].str.contains("Unit|Timestamp")]
    data = data[data["Timestamp"].notna()]
    data["Timestamp"] = pd.to_datetime(data["Timestamp"])
    data["Nitrogen01"] = pd.to_numeric(data["Nitrogen01"], errors="coerce")
    data = data[data["Nitrogen01"] > 0]
...
def perform_eda(data, location):
    ...
    ax_ts.plot(location_data["Timestamp"], location_data["Nitrogen01"], marker="o")
    ax_ts.set_xlabel("Timestamp")
    ax_ts.set_ylabel("Nitrogen")
    ax_ts.set_title(f"Time Series Plot for {location}")
    ax_ts.grid(True)

    # ADF Test
    result = adfuller(location_data["Nitrogen01"].dropna())
    adf_results = {
    ...
    decomposition = seasonal_decompose(
        location_data.set_index("Timestamp")["Nitrogen01"], model="additive", period=4
    )
...
def fit_arima_model(data, location, order):
    location_data = data[data["Location"] == location]["Nitrogen01"]
...
def forecast_and_plot(model, data, location, steps=25):
    ...
    plt.plot(
        ...
        location_data["Nitrogen01"],
        marker="o",
        label=f"Actual {location} Nitrogen",
    )
    plt.plot(
        next_dates,
        forecast,
        ...
        marker="o",
        color="red",
        label=f"Forecast {location} Nitrogen",
    )
...
def main():
    ...
    for location in locations:
        ...
        print(f"Forecasted Nitrogen values for {location}:")
