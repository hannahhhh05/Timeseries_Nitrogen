...
from schedule_nitrogen_timeseries import load_and_concat_all_sheets_in_centers, clean_data, perform_eda, fit_arima_model, forecast_and_plot

st.set_page_config(page_title="Nitrogen Forecast App", layout="wide")

st.title("Nitrogen Forecast App")
...
st.header(f"Nitrogen Forecast for {selected_location}")
...
plt.plot(location_data['Timestamp'], location_data['Nitrogen01'], marker='o', label=f'Actual {selected_location} Nitrogen')
plt.plot(dates, forecast, linestyle='--', marker='o', color='red', label=f'Forecast {selected_location} Nitrogen')
...
forecast_df = pd.DataFrame({'Date': dates, 'Forecasted Nitrogen': forecast})
...
st.download_button(
    ...
    data=csv,
    file_name=f"{selected_location}_Nitrogen_forecast.csv",
    mime="text/csv",
)
